...
- [ ] 2REC.1. Write GraphQL query — fetch recipes list — **depends on 1INF.7, 1DB.4**
...
_(unblocks once 1INF.7 and 1DB.4 are complete)_
...
- [ ] 3HH.1. Create household flow (name, create, redirect to plan) — **depends on 1DB.2**
- [ ] 3HH.2. Generate unique invite code for a household — **depends on 3HH.1**
...
- [ ] 3RL.1. Role enum in DB (Leader / Contributor / Member) — **depends on 1DB.2**
- [ ] 3RL.2. Role-based UI guards (hide edit/delete for Member role) — **depends on 3RL.1, 3HH.1**
- [ ] 3RL.3. Assign role when inviting or accepting a member — **depends on 3HH.3, 3RL.1**

<a name="m3-blocked"><h4>Blocked (Milestone 3)</h4></a>

_(unblocks once 1DB.2 is complete)_
...
- [ ] 4PL.2. GraphQL query — fetch meal plan for a given week — **depends on 1DB.4, 3HH.1**
...
_(unblocks once M2 REC and M3 HH tasks are complete)_
...
- [ ] 5SH.1. GraphQL queries + mutations for shopping list items — **depends on 1DB.4**
...
_(unblocks once M4 plan tasks and 1DB.4 are complete)_
...
"1INF.7"["`*1INF.7*<br/>**INF**<br/>Wire ApolloProvider`"]:::open
"1INF.8"["`*1INF.8*<br/>**INF**<br/>App metadata`"]:::open
"1NAV.1"["`*1NAV.1*<br/>**NAV**<br/>Nav component`"]:::open
"1NAV.2"["`*1NAV.2*<br/>**NAV**<br/>Nav routes`"]:::blocked
"1DB.1"["`*1DB.1*<br/>**DB**<br/>Design schema`"]:::open
"1DB.2"["`*1DB.2*<br/>**DB**<br/>Create tables`"]:::blocked
"1DB.3"["`*1DB.3*<br/>**DB**<br/>RLS policies`"]:::blocked
"1DB.4"["`*1DB.4*<br/>**DB**<br/>Verify GraphQL`"]:::blocked
...
%% ─── Milestone 2: Recipes ────────────────────────────────────────────────────
"2REC.1"["`*2REC.1*<br/>**REC**<br/>Query recipes`"]:::blocked
...
"3HH.1"["`*3HH.1*<br/>**HH**<br/>Create household`"]:::blocked
...
"3RL.1"["`*3RL.1*<br/>**RL**<br/>Role enum in DB`"]:::blocked
...
"5SH.1"["`*5SH.1*<br/>**SH**<br/>List queries & mutations`"]:::blocked
